...
class PeerConnectionManager @Inject constructor(
    ...
    /**
     * Creates exactly one [PeerConnection] per session.
    ...
    fun createPeerConnection(
        ...
        if (pc == null) {
            Timber.e(
                "WEBRTC createPeerConnection returned null (control=%s emulator=%s)",
                isControlDevice,
    ...
    /**
     * Applies a remote answer only when the peer is waiting for one (have-local-offer).
     * Duplicate or stale answers received in stable state are ignored without error.
     */
    fun applyRemoteAnswer(
        ...
        return when (pc.signalingState()) {
            ...
            PeerConnection.SignalingState.STABLE -> {
                Timber.w(
                    "Ignoring unexpected/duplicate answer. state=%s",
                    pc.signalingState()
                )
                false
            }
